...
var Dynamic = ( function () {
    ...
    devDynamicContent.Ernest_Jones_Gift_Finder_creativeFeed[0].headlineCopy = "GIFTS FOR THE ONE YOU LOVE";
			...
			e.target.style.backgroundColor = "#ffffff";
						...
						if(e.target.dataset.getClick == 'false'){
								...
								if(isFirst){
									...
									gsap.to(DOM.get().buttonContainer, {y:-153,zIndex:999,flexDirection:'row',justifyContent:'space-evenly',width:300,height:30,gap:3});
									gsap.to([DOM.get().buttons], {fontSize:13,padding:'0px 0px',minHeight:30,width:85,height:27,delay:.1, duration:.3});
									DOM.get().buttons.forEach((element, index) => {
								
										let btnSpan = element.firstElementChild;
		
										gsap.set(element.firstElementChild, {width:75});
...
function buildCarousel(checkEligibleProduct) {
  ...
  var sliderSize = carouselSize;
  var productSize = sliderSize;
...
function populateCarousel(content) {
	...
	for (var i = 0; i < (productEligible.length); i++) {
	  ...
	  if (productEligible[productNumber].productSalePrice) {
		...
		gsap.set('.nmpiProductPrice.size300x250 span', {position:'relative', top:'unset',x:0,y:0,left:'unset'});
		gsap.set(productPriceSpan, {paddingRight:3});
	  }
