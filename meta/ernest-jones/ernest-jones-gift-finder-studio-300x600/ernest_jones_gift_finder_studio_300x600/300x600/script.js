			...
			e.target.style.backgroundColor = "#ffffff";
				...
				if(e.target.dataset.getClick == 'false'){
						...
						if(isFirst){
							...
							gsap.to(DOM.get().buttonContainer, {y:-468,zIndex:999,flexDirection:'row',width:285,height:30});
							gsap.to([DOM.get().buttons], {padding:'3px 0px',width:90,delay:.1, duration:.3});
							
							DOM.get().buttons.forEach((element, index) => {
								
								let btnSpan = element.firstElementChild;

								gsap.set(element.firstElementChild, {width:80});
...
function populateCarousel(content) {
  ...
  for (var i = 0; i < (productEligible.length); i++) {
	...
	if (productEligible[productNumber].productSalePrice) {
		...
		gsap.set('.nmpiProductPrice.size300x600 span', {position:'relative', top:'unset',x:0,y:0,left:'unset'});
		gsap.set(productPriceSpan, {paddingRight:15});
	  }
