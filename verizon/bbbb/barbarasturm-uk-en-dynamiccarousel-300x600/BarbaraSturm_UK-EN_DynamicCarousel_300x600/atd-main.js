...
var creative = (function () {
	...
	_this.init = function () {
		...
		gsap.set('.loading_screen', {opacity:0});
...
var carousel = (function () {
	...
	_this.configureCarousel = function (options) {
		...
		carouselItemHeight = options.itemHeight || 147;
		carouselImageWidth = options.imageWidth || 139;
		carouselImageHeight = options.imageHeight || 110;
	...
	_this.generateCarousel = function () {
					...
					<div class="carousel-product-info-wrapper">
						...
						<div class="carousel-product-prices">
							<div class="carousel-product-sale-price">
								<div class="price">${removeDecimal(productArr[i].salePrice)}</div>
							</div>
							<div class="carousel-product-price">
							<div class="price">${removeDecimal(productArr[i].price)}</div>
						</div>
						</div>
		...
		function removeDecimal(input) {
			// Use a regular expression to extract the non-numeric prefix and numeric part
			let match = input.match(/([^\d]*)(\d+\.?\d*)/);
			if (match) {
				let prefix = match[1] || "";
				let numericPart = parseFloat(match[2]);
		
				// Convert the numeric part to an integer
				let integerPart = Math.floor(numericPart);
		
				// Concatenate the prefix with the integer value
				return prefix + integerPart;
			} else {
				return input; // If no match, return the input unchanged
			}
		}
	};
	...
	_this.checkOnSale = function () {
		for (let i = 0; i < carouselItemsArr.length; i++) {
			...
			if (saleHolder.innerHTML != "") {
				...
				gsap.set(saleHolder.parentNode, {display:'flex'});
