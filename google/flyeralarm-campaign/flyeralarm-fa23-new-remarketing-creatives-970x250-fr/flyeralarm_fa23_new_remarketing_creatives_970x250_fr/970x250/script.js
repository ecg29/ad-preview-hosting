...
addListeners = function (e) {
    ...
    document.querySelector('#replay_btn').addEventListener('click', function (e) {
        ...
        slideIndex = 1;
        previousImg = 0;
        gsap.set('#background-endframe', { clipPath: 'inset(0 0 0 978px)' });
...
addAnimation = function (e) {
    ...
    tl.to('#logo-wrapper', { autoAlpha: 1, duration: 0.5 })
        ...
        .to('#logo-container', { x: -464, y: -20, scale: 0.55, delay: 1, duration: 2.1, ease: "power3.out" }, "logoSplit")
...
function showSlides(n, direction) {
    ...
    if (direction == 'right') {
        ...
        tl.fromTo(slides[slideIndex - 1], .8, { rotation: 0.01, x: 970, scale: .8, autoAlpha: 0, zIndex: 1 }, {
        ...
        if (previousImg != 0) {
            TweenMax.to(slides[previousImg - 1], .8, { x: -970, scale: .8, autoAlpha: 1, zIndex: 1, onComplete: function () { onAnim = false } });
