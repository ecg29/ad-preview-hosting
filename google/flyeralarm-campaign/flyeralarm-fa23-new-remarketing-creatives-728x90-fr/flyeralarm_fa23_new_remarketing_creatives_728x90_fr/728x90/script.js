...
addListeners = function (e) {
    ...
    document.querySelector('#replay_btn').addEventListener('click', function (e) {
        ...
        slideIndex = 1;
        previousImg = 0;
        gsap.set('#background-endframe', { clipPath: 'inset(0 0 0 718px)' });
...
addDynamicContent = function (e) {
    ...
    // textFit(document.querySelectorAll('.product-title-copy'));
...
addAnimation = function (e) {
    ...
    tl.to('#logo-wrapper', { autoAlpha: 1, duration: 0.5 })
        .to('.slideshow-container', { y: 0, duration: 1.1, ease: 'sine.out' })
        .to('#logo-container', { x: -348, y: 0, scale: 1, delay: 1, duration: 1.7, ease: "power3.out" }, "logoSplit")
        ...
        .from(hllines, {
            ...
            ease: "back.out",
            stagger: { each: 0.04 }, onStart: () => gsap.set('#headline-container', { autoAlpha: 1 })
        })
        .to('.slideshow-container', {
            clipPath: 'inset(0 0 0 0px)', delay: 1, duration: 1.3, ease: 'sine.out', onStart: function () {

                gsap.to('#background-endframe', { y: 0, duration: 1.1, ease: 'sine.out' });

                var imgAnim = gsap.timeline();

                imgAnim.to(document.querySelectorAll('.slide-panel')[0], {
                    rotation: 0.1, delay: 0.3, duration: 3, y: '-60%', ease: 'power0.none', onComplete: function () {
                    }
                })
                    .to(document.querySelectorAll('.slide-panel')[0], {
                        rotation: 0.1, delay: 0.3, duration: 1.5, y: '-50%', ease: 'power0.none', onComplete: function () {
...
function plusSlides(n, direction) {
    ...
    // rotateInterval = gsap.delayedCall(2.5, rotateOnce);
}
...
function currentSlide(n) {
    ...
    for (var j = 0; j < slides.length; j++) {
        TweenMax.set(slides[j], { x: 0, scale: 1, autoAlpha: 0, zIndex: 5 })

    }
    TweenMax.set(slides[n - 1], { x: 0, scale: 1, zIndex: 6 });
    TweenMax.set(slides[n - 1], { autoAlpha: 1, zIndex: 6 });
...
function showSlides(n, direction) {
    ...
    if (direction == 'right') {
        ...
        tl.fromTo(slides[slideIndex - 1], .8, { rotation: 0.01, x: 728, scale: .8, autoAlpha: 0, zIndex: 1 }, {
            x: 0, scale: 1, autoAlpha: 1, onComplete: function () {
                onAnim = false
        ...
        if (previousImg != 0) {
            TweenMax.to(slides[previousImg - 1], .8, { x: -728, scale: .8, autoAlpha: 1, zIndex: 1, onComplete: function () { onAnim = false } });
    ...
    if (direction == 'left') {
        ...
        tl.fromTo(slides[slideIndex - 1], .8, { x: -728, scale: .8, opacity: 0, zIndex: 1 }, { x: 0, scale: 1, opacity: 1, onComplete: function () { onAnim = false }, zIndex: 5 }, "init")
        if (previousImg != 0) {
            TweenMax.to(slides[previousImg - 1], .8, { x: 728, scale: .8, opacity: 1, onComplete: function () { onAnim = false }, zIndex: 1 });
...
function rotateOnce() {
    ...
    if (previousImg == 2 && !firstRotate) {
        ...
        gsap.to('#background-endframe', {
            delay: 4.5, clipPath: 'inset(0 0 0 0px)', duration: 1.3, ease: 'sine.out', onComplete: function () {
...
function pageClick(isPageClick) {
    ...
    if (isPageClick) {
        gsap.set('.slide-panel', { y: '-50%' })
        // rotateInterval.kill();
        gsap.killTweensOf(rotateOnce);
        pageTween = gsap.delayedCall(2.5, function () {
            gsap.to('#background-endframe', {
                clipPath: 'inset(0 0 0 0px)', duration: 1.3, ease: 'sine.out', onComplete: function () {
                    gsap.to('#logo-wrapper-endframe,.product-cta-endframe,#replay_btn', { stagger: 0.2, delay: 0.8, autoAlpha: 1, duration: 0.5 })
                }
            });
        });
